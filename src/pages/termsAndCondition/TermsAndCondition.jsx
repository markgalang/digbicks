import React from "react";
import "./TermsAndCondition.css";
import DigBicks from "assets/images/logo.png";
import Footer from "components/Footer/Footer";
import { Link } from "react-router-dom";

function TermsAndCondition() {
  return (
    <div className="termsContainer">
      <Link to="/">
        <img src={DigBicks} className="termsLogo" alt="DigBicks" />
      </Link>

      <div className="termsContentContainer">
        <div className="termsTitle">Terms and Condition</div>
        <p>
          DigBicks is a collection of digital artworks (NFTs) running on the
          Ethereum network. This website is only an interface allowing
          participants to exchange digital collectibles. Users are entirely
          responsible for the safety and management of their own private
          Ethereum wallets and validating all transactions and contracts
          generated by this website before approval. Furthermore, as the
          DigBicks smart contract runs on the Ethereum network, there is no
          ability to undo, reverse, or restore any transactions.
        </p>
        <br />
        <p>
          This website and its connected services are provided “as is” and “as
          available” without warranty of any kind. By using this website you are
          accepting sole responsibility for any and all transactions involving
          DigBicks digital collectibles.
        </p>

        <br />
        <br />
        <h2>OWNERSHIP</h2>
        <br />
        <p>
          i. You Own the NFT. Each DigBicks is an NFT on the Ethereum
          blockchain. When you purchase an NFT, you own the underlying DigBicks,
          the Art, completely. Ownership of the NFT is mediated entirely by the
          Smart Contract and the Ethereum Network: at no point may we seize,
          freeze, or otherwise modify the ownership of any DigBicks.
        </p>
        <br />
        <p>
          ii. Personal Use. Subject to your continued compliance with these
          Terms, DigBicks Army grants you a worldwide, royalty-free license to
          use, copy, and display the purchased Art, along with any extensions
          that you choose to create or use, solely for the following purposes:
          (i) for your own personal, non-commercial use; (ii) as part of a
          marketplace that permits the purchase and sale of your DigBicks / NFT,
          provided that the marketplace cryptographically verifies each DigBicks
          owner’s rights to display the Art for their DigBicks to ensure that
          only the actual owner can display the Art; or (iii) as part of a third
          party website or application that permits the inclusion, involvement,
          or participation of your DigBicks, provided that the
          website/application cryptographically verifies each DigBicks owner’s
          rights to display the Art for their DigBicks to ensure that only the
          actual owner can display the Art, and provided that the Art is no
          longer visible once the owner of the DigBicks leaves the
          website/application.
        </p>
        <br />
        <p>
          iii. Commercial Use. Subject to your continued compliance with these
          Terms, DigBicks Army grants you an unlimited, worldwide license to
          use, copy, and display the purchased Art for the purpose of creating
          derivative works based upon the Art (“Commercial Use”). Examples of
          such Commercial Use would e.g. be the use of the Art to produce and
          sell merchandise products (T-Shirts etc.) displaying copies of the
          Art. For the sake of clarity, nothing in this Section will be deemed
          to restrict you from (i) owning or operating a marketplace that
          permits the use and sale of DigBicks generally, provided that the
          marketplace cryptographically verifies each DigBicks owner’s rights to
          display the Art for their DigBicks to ensure that only the actual
          owner can display the Art; (ii) owning or operating a third party
          website or application that permits the inclusion, involvement, or
          participation of DigBicks generally, provided that the third party
          website or application cryptographically verifies each DigBicks
          owner’s rights to display the Art for their DigBicks to ensure that
          only the actual owner can display the Art, and provided that the Art
          is no longer visible once the owner of the Purchased DigBicks leaves
          the website/application; or (iii) earning revenue from any of the
          foregoing.
        </p>
      </div>

      <Footer />
    </div>
  );
}

export default TermsAndCondition;
